...
public class GenerateDungeon
{
	...
	private final  DungeonTile[][] dungeon;
	...
	private static int[] checkIfGoodPath(int xLoc, int yLoc, int xPrev, int yPrev, int[] limit, DungeonTile[][]
			                                                                                              dungeon,
	                                     int[][] checked, int[] startEnd)
	...
	private static int findIfValid(int xLoc, int yLoc, DungeonTile[][] dungeon, int[][] checked, int[] startEnd)
	throws StackOverflowError
	{
		int value = 0;
		countToMax++;
		...
		if(checked[yLoc][xLoc] == 0)
		{
			...
			if(value == 0 && xLoc + 1 != dungeon[0].length && ! dungeon[yLoc][xLoc + 1].isUsed() && checked[yLoc][xLoc +
					                                                                                                        1] == 0)
			{
				value = findIfValid(xLoc + 1, yLoc, dungeon, checked, startEnd);
			}
			if(value == 0 && yLoc + 1 != dungeon.length && ! dungeon[yLoc + 1][xLoc].isUsed() && checked[yLoc + 1][xLoc]
					                                                                                       == 0)
			{
